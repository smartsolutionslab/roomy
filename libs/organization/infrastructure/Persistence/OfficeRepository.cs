...
{
    public async Task<Result<Office>> GetByIdentifierAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (office is null)
            return Error.NotFound("office.not_found", $"No office exists with identifier '{identifier}'.");
    ...
    public Task<bool> ExistsByNameAsync(
        ...
        OfficeName name,
        CancellationToken cancellationToken) =>
        context.Offices.AnyAsync(
            office => office.CompanyIdentifier == companyIdentifier && office.Name == name,
            cancellationToken);
