...
/// covering the issue #19 Done criteria: append + load-by-replay works, and a concurrency conflict
/// is rejected. The unique <c>(stream_id, version)</c> index is created from the EF model, so the
/// conflict is enforced by the database exactly as it will be on PostgreSQL (ADR-0012).
/// </summary>
public sealed class EfCoreEventStoreTests
{
    ...
    [Fact]
    public async Task Concurrent_appends_on_the_same_expected_version_let_only_one_win()
    {
        using var fixture = new SqliteEventStoreFixture();
        var streamId = StreamId.From(Guid.NewGuid());
        var booked = new DeskBooked(Guid.NewGuid(), "ada", new DateOnly(2026, 6, 8));
        ...
        // The second writer read version 0 too, but the unique (stream_id, version) index rejects it.
