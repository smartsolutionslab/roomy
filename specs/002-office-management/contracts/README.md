...
added to `libs/organization/contracts` (the organization context's *published language*, ADR-0031)
and emitted over the transactional outbox in a dedicated slice.

The `EmployeeHired`/`HiredRole` records already in `libs/organization/contracts` belong to the
