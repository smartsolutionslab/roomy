...
- [x] T014 [P] [US6] RED application tests in `tests/attendance/Application/ViewOccupancyTests.cs` — room capacity 8 + 3 rows ⇒ `3/8` (scenario 1); rooms Σcap 30 + 12 rows ⇒ rollup `12/30`, a 0-reservation room still counted (scenario 2, edge); each day in `[from,to]` returns its own figure incl. past dates (scenarios 3, 8); `occupied==capacity` ⇒ `isFull` (scenario 9); **names present only for today + tomorrow**, counts only otherwise, pinned via a fake `TimeProvider` Europe/Berlin (scenario 4, FR-007).
- [x] T015 [US6] Implement `ViewOccupancy` query + `OccupancyView`/`OfficeOccupancy`/`RoomOccupancy` DTOs (`libs/attendance/application/UseCases/`), the `IOccupancyReadModel` port (`.../Ports/`), and the today/tomorrow name policy in the handler (`TimeProvider`, research R7). T014 green.
