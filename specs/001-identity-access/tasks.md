...
- [ ] T010 EF Core persistence in `libs/identity/infrastructure/Persistence/` — `IdentityDbContext` + `User` mapping (unique index on `Email` and `KeycloakSubjectId`) + initial migration; integration test in `tests/identity/Infrastructure/PersistenceTests.cs` (Testcontainers Postgres), written failing first.
