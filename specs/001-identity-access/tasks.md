...
- [x] T023 [US4] RED-verified across the layers the behaviour actually spans (the single-file guess in this task predated the layering): domain elevation + the `AdministratorGranted` event in `tests/identity/Domain/Users/UserTests.cs`; the `GrantAdministrator` use case (elevate → Keycloak sync → persist, idempotent, not-found, sync-failure) in `tests/identity/Features/AdminManagementTests.cs` with in-memory port doubles; and the HTTP surface — `/admin/users`, `/admin/users/{id}`, `:grant-administrator`, plus `403` for employees and `401` without a session — in `tests/identity-integration/AdminUserEndpointsTests.cs` (in-process host, real Postgres, Keycloak stubbed). The real Keycloak role-assignment round-trip is `tests/identity-integration/KeycloakIdentityProviderTests.cs`.
- [x] T024 [US4] `AdministratorGranted` is an **intra-context domain event** (no cross-context consumer — absent from `integration-events.md`), so it needed a domain-events mechanism the codebase did not yet have. Added it under **ADR-0032**: a shared-kernel `IDomainEvent` marker + an abstract `Aggregate` base that records events (`DomainEvents` / `RaiseDomainEvent` / `ClearDomainEvents`); `User` now derives from `Aggregate`. `User.GrantAdministrator(occurredAt)` (in `libs/identity/domain/Users/User.cs`) elevates the `Role` and raises `AdministratorGranted` — idempotent: an account already an administrator changes nothing and raises nothing. `OccurredAt` is supplied by the caller's clock, never read ambiently. Events are collected, not dispatched (no consumer yet); EF ignores the `DomainEvents` collection. T024 green.
- [x] T025 [US4] `GrantAdministrator` command + `GrantAdministratorHandler` in `libs/identity/application/UseCases/`: load → not-found is `Error.NotFound` (→ 404) → already-administrator is an idempotent no-op → else **provider-first** (Keycloak is the token authority, ADR-0013) assign the realm role via the new `IIdentityProviderPort.AssignAdministratorRoleAsync` (Keycloak adapter implemented + verified against real Keycloak), then raise the elevation and commit. Persistence outside the messaging pipeline needed a commit seam, so a small owned `IUnitOfWork` port (`libs/identity/application/IUnitOfWork.cs`) with an EF adapter (`IdentityUnitOfWork`) was added and wired; the handler is bound via `AddIdentityUseCases` (DI registration test guards it). The `POST /admin/users/{userId}:grant-administrator` endpoint maps the result to `204`/`404`. T025 green.
- [x] T026 [US4] `GET /admin/users` and `GET /admin/users/{userId}` in `apps/identity-api/Endpoints/AdminUserEndpoints.cs` (+ `AdminUserResponse`, which adds `status` over the account/me projection; + `IUserRepository.GetAllAsync`). All `/admin/*` routes require the `administrator` role (`RequireRole` → `403` for employees, FR-007; `401` without a session). Keycloak nests realm roles under `realm_access.roles`, so the host flattens them to `ClaimTypes.Role` on the BFF-forwarded token (`KeycloakRealmRoles`, unit-tested; wired in the JWT-bearer `OnTokenValidated`); the in-process tests issue role claims through the test auth scheme, and the real-token mapping is verified by `KeycloakRealmRolesTests`. T026 green.
