...
- Paths follow plan.md: `backend/libs/organization/{domain,application,infrastructure}/`, `backend/apps/organization-api/`, `backend/apps/gateway/`, `backend/tests/organization*/`, consuming `backend/libs/identity/contracts`, publishing `backend/libs/organization/contracts`.
...
- [x] T001 Add a `ProjectReference` from `backend/libs/organization/infrastructure` to `backend/libs/identity/contracts` so the new consumers can map identity's `UserRegistered`/`UserProvisioningFailed` (`context:shared`, ADR-0031); confirm the Nx tags still pass the boundary lint and that `Roomy.ArchitectureTests` already references the three organization projects (added in 002 — otherwise the rules are vacuous).
...
- [x] T007 [P] Confirm the architecture suite stays green — `Employee` lives in `Roomy.Organization.Domain`; the foreign identity contracts are referenced only at the infrastructure edge, never in `application` (`backend/tests/architecture`).
...
- [x] T008 [P] [US1] RED application tests in `backend/tests/organization/Application/` — `HireEmployeeHandler` gets the seeded company, **pre-allocates a `UserId`**, creates the employee, and commits once (mirrors `CreateOfficeHandler`); and a mapping test that `OrganizationIntegrationEventMap` turns the `EmployeeHired` domain event into the `EmployeeHired` **contract** carrying the pre-allocated `UserId` and the password.
- [x] T009 [US1] Implement `HireEmployee` command + `HireEmployeeHandler` (`backend/libs/organization/application/UseCases/`) and EXTEND `OrganizationIntegrationEventMap` with the `EmployeeHired` domain→contract mapping (`backend/libs/organization/infrastructure/Messaging/`); register the handler + repository in `OrganizationInfrastructureServiceCollectionExtensions`. T008 green.
- [x] T010 [US1] RED application test for the `UserRegistered` ack path; implement `CompleteEmployeeProvisioning` command + handler (`backend/libs/organization/application/UseCases/`) and `UserRegisteredConsumer` (`backend/libs/organization/infrastructure/Messaging/`) mapping `Contracts.Identity.UserRegistered` → the command (employee ⇒ `Active`).
- [x] T011 [US1] Make organization-api a **consumer**: in `backend/apps/organization-api/Program.cs` pass the organization **infrastructure** assembly to `AddRoomyMessaging` so the inbox scans the consumers (it stays a publisher via `AddIntegrationEventOutbox`). Regenerate the committed Wolverine codegen (`backend/apps/organization-api/Internal/Generated/`); confirm organization-api is in the CI `codegen verify` step ([[wolverine-codegen-platform-sensitive]]).
- [x] T012 [US1] RED API/contract tests (`backend/tests/organization-integration/`, WebApplicationFactory) for `POST /employees` — **202** `{ employeeId, userId, state: "Provisioning" }`; **422 `invalid_hire`** (bad email/empty name/unknown role/empty password); **403** non-admin; **401** no session — per `contracts/organization-api.md`. Implement `EmployeeEndpoints` (`backend/apps/organization-api/Endpoints/`, admin-only), register in `Program.cs`, add the `/employees` route to `backend/apps/gateway/appsettings.json`, re-emit the OpenAPI spec (`Roomy.Organization.Api.json`) and confirm/add organization-api to the CI OpenAPI drift-gate step. T012 green.
- [~] T013 [US1] **Integration test (DEFERRED to a full-stack e2e harness)** — the happy-path round-trip needs **both** API hosts + RabbitMQ + Keycloak running together; the repo's integration fixtures are Postgres-only. Every constituent piece is covered: org publishes `EmployeeHired` (drain test), identity consumes it → `RegisterUser` → `UserRegistered` (identity-integration tests on `main`), org consumes the ack → `Active` (handler + consumer unit tests), endpoint (contract tests). The cross-process transport is the same proven Wolverine path that already moves `OfficeOpened`/`RoomAdded`. Tracked for the e2e suite (ADR-0022); verifiable now via the quickstart manual smoke.
...
- [~] T016 [US3] (DEFERRED to e2e; terminal-state idempotency is unit-tested in EmployeeTests, inbox dedup is Wolverine) **Integration test** — re-deliver `EmployeeHired` and re-deliver the `UserRegistered`/`UserProvisioningFailed` acks; assert exactly one employee, one login account, and a single terminal transition (the inbox de-duplicates and `CompleteProvisioning`/`FailProvisioning` are terminal-state no-ops, FR-008). A new hire reusing an existing email resolves to *failed* (US2), never a silent duplicate.
