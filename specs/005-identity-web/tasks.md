...
- [ ] T001 [P] Typed view models + data-access clients in `apps/web/src/app/identity/` — `account.ts`
...
- [ ] T002 [P] Extend `SessionService` (D-FE3) with a readiness signal so guards can await the first
  session resolution; `load()` flips it once `/bff/user` resolves (success or 401). Spec covers both.
- [ ] T003 Add the gateway `/admin/{**}` YARP route in `apps/gateway/appsettings.json` (mirror
...
- [ ] T004 [US1] `account-page.spec.ts` (RED) — renders the account from a stubbed `account-client`
  (name, email, role label); role renders localized; loading + error states. `@testing-library/angular`.
- [ ] T005 [US1] `account-page.ts/.html/.css` (standalone, OnPush, signal-based; `TranslocoDirective`),
...
- [ ] T006 [US2] `auth.guard.spec.ts` (RED) — null session → redirects browser to
  `/bff/login?returnUrl=<path>`; present session → allows. Awaits session readiness (T002).
- [ ] T007 [US2] `auth.guard.ts` (functional `CanActivateFn`), applied to `/account` (and later
  `/admin`). T006 green.
...
- [ ] T008 [US3] `admin.guard.spec.ts` (RED) — roles include `administrator` → allows; otherwise →
...
- [ ] T009 [US3] `admin.guard.ts`, `admin-users-page.ts/.html/.css`, `not-authorized.ts/.html`, the
...
- [ ] T010 [US4] Extend `admin-users-page.spec.ts` (RED) — clicking "Grant administrator" calls the
...
- [ ] T011 [US4] Wire the grant action in `admin-users-page.ts` (+ confirm affordance, `aria-live`
...
- [ ] T012 [P] Accessibility pass (WCAG 2.2 AA): keyboard operability, focus visibility, roles/names,
