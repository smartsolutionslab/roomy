...
public static class GuardExtensions
{
    public static Guard<T> IsNotNull<T>(this Guard<T?> guard)
        where T : class
    {
        if (guard.Value is null) throw new ArgumentNullException(guard.Name);
    ...
    {
        if (string.IsNullOrEmpty(guard.Value)) throw new ArgumentException("Value must not be empty.", guard.Name);
    ...
    {
        if (string.IsNullOrWhiteSpace(guard.Value)) throw new ArgumentException("Value must not be null or whitespace.", guard.Name);
    ...
    {
        if (guard.Value <= 0) throw new ArgumentOutOfRangeException(guard.Name, guard.Value, "Value must be positive.");
    ...
    {
        if (!predicate(guard.Value)) throw new ArgumentException(message, guard.Name);
