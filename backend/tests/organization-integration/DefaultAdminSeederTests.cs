...
public sealed class DefaultAdminSeederTests
{
    ...
    public async Task Hires_the_default_admin_as_an_administrator_when_absent()
    {
        ...
        await new DefaultAdminSeeder(new StubEmployees(exists: false), hire, Options())
    ...
    [Fact]
    public async Task Does_not_hire_again_when_the_admin_already_exists()
    {
        var hire = new RecordingHire();

        await new DefaultAdminSeeder(new StubEmployees(exists: true), hire, Options())
            .SeedAsync(CancellationToken.None);
